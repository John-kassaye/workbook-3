...
public class date {
    ...
    public static void main(String[] args) {
       ...
       // #18
        ...
        //#19
        LocalDate localDate3 = LocalDate.of(2022,3,12);
        ...
        LocalDate localDate5 = LocalDate.parse(date,dtf4);

        long daysLeft = ChronoUnit.DAYS.between(localDate, LocalDate.now());

        if (localDate5.isBefore(LocalDate.now())){
            System.out.println("The event was " + Math.abs(ChronoUnit.DAYS.between(localDate5,LocalDate.now())) + " days ago");
        }else if(localDate5.isAfter(LocalDate.now())){
            System.out.println(Math.abs(ChronoUnit.DAYS.between(localDate5,LocalDate.now())) + " days left");
        }

        //#25

        LocalDate localDate6 = LocalDate.of(2025,04,25);
        LocalTime localTime = LocalTime.of(3,45);
        LocalDateTime localDateTime6 = LocalDateTime.of(localDate6 , localTime);
        LocalDateTime result = null;
        for (int i = 0; i < 4; i++){
            System.out.println(localDateTime6.plusWeeks(i));
        }
